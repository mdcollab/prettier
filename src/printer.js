...
function genericPrintNoParens(path, options, print, args) {
  ...
  switch (n.type) {
    ...
    case "ConditionalExpression": {
      const printedConsequent = path.call(print, "consequent");
      const printedAlternate = path.call(print, "alternate");
      const consequentOrAlternateWillBreak = willBreak(printedConsequent) || willBreak(printedAlternate);
      ...
      return group(
        concat([
          ...
          indent(
            concat([
              consequentOrAlternateWillBreak? hardline: line,
              printedConsequent,
              ":",
              consequentOrAlternateWillBreak? hardline: line,
              printedAlternate
    ...
    case "JSXOpeningElement": {
      ...
      const attrs = path.map(attr => concat([hardline, print(attr)]), "attributes");
      const hasSpread = path.getNode()
        .attributes
        .filter(({type}) => type === "JSXSpreadAttribute")
        .length > 0;
      ...
      return group(
        concat([
          ...
          path.call(print, "name"),
          concat([
            indent(concat(hasSpread? attrs: sortJSXAttrs(attrs))),
...
function printMemberChain(path, options, print) {
  ...
  function printIndentedGroup(groups, noIndent) {
    ...
    const printed = group(concat([hardline, join(hardline, groups.map(printGroup))]));
    return noIndent? printed: indent(printed);
  }
...
function printAssignment(
  ...
  const canBreak =
    (isBinaryish(rightNode) && !shouldInlineLogicalExpression(rightNode, path)) ||
    (rightNode.type === "ConditionalExpression" &&
      isBinaryish(rightNode.test) &&
      !shouldInlineLogicalExpression(rightNode.test, path)) ||
